...
class Cart extends Tools_Cart_Cart {
	...
	protected function _renderLandingForm($signupForm = null) {
		...
		if ($flashMessenger) {
			...
			if (!empty($msg) && (in_array('There is no user with such login and password.', $msg[0]) || in_array('Login should be a valid email address', $msg[0]) || in_array('Value is required and can\'t be empty', $msg[0]) || preg_match('~sent an email to~',$msg[0]))) {
